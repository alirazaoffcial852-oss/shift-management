...
import WarehouseShiftStatusLegend from "./WarehouseShiftStatusLegend";

export default function WarehouseShiftsWeeklyView() {
  const {
    currentDate,
    selectedShifts,
    shifts,
      ...
      shift.usn_shift_roles?.some((role) => {
        ...
        if (employeeId !== null) {
          return role.usn_shift_personnels?.some(
            (personnel: { employee_id: any }) =>
              Number(personnel.employee_id) === employeeId
          );
        }
