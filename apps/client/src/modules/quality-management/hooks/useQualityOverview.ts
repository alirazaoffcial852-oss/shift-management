...
export type UseQualityOverviewState = {
  search: string;
...
const initialState: UseQualityOverviewState = {
  search: "",
  dateRange: {},
  filters: {},
  page: 1,
...
export function useQualityOverview() {
  const [state, setState] =
    React.useState<UseQualityOverviewState>(initialState);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const res = await fetchQualityOverview(query);
      setData(res);
    } catch (e: any) {
