...
export type UseCompanyFeedbacksState = {
  search: string;
...
const initialState: UseCompanyFeedbacksState = {
  search: "",
  dateRange: {},
  filters: {},
  page: 1,
...
export function useCompanyFeedbacks() {
  const [state, setState] =
    React.useState<UseCompanyFeedbacksState>(initialState);
  const [data, setData] = React.useState<CompanyFeedbackResponse>({
    rows: [],
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    setError(null);
    const query: CompanyFeedbackQuery = {
    ...
    try {
      const res = await fetchCompanyFeedbacks(query);
      setData(res);
    } catch (e: any) {
      setError(e?.message || "Failed to load company feedbacks");
    } finally {
      setLoading(false);
    }
  }, [
