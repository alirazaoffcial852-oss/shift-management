...
export type UseCustomerFeedbacksState = {
  search: string;
...
const initialState: UseCustomerFeedbacksState = {
  search: "",
  dateRange: {},
  filters: {},
  page: 1,
...
export function useCustomerFeedbacks() {
  const [state, setState] =
    React.useState<UseCustomerFeedbacksState>(initialState);
  const [data, setData] = React.useState<CustomerFeedbackResponse>({
    rows: [],
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    setError(null);
    const query: CustomerFeedbackQuery = {
    ...
    try {
      const res = await fetchCustomerFeedbacks(query);
      setData(res);
    } catch (e: any) {
      setError(e?.message || "Failed to load customer feedbacks");
    } finally {
      setLoading(false);
    }
  }, [
