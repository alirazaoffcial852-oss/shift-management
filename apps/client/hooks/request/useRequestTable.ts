...
export const useRequestTable = (initialPage = 1, limit = 20) => {
  ...
  const updateTimeSheetStatus = useCallback((timeSheetId: number[]) => {
    setTimeSheets((prevProjects) => prevProjects.filter((project) => !timeSheetId.some((timesheet) => timesheet?.toString() === project.id?.toString())));
  }, []);
