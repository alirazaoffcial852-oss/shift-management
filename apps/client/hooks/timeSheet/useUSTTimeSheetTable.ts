...
export const useUSTimeSheetTable = (initialPage = 1, limit = 20) => {
  ...
  const fetchUSTimeSheet = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    try {
      ...
      const response = await TimeSheetService.getUSTimeSheet(
        currentPage,
        limit,
        status,
        company?.id.toString(),
      ...
      // Handle response structure: response.data.data contains the array
      ...
      const errorMsg = err.message || "Failed to fetch USN timeSheets";
      setError(errorMsg);
      console.error("Error fetching USN timeSheets:", err);
      return null;
    } finally {
      setIsLoading(false);
    }
  }, [currentPage, limit, company, status, searchParams, employeeId, isClient]);
