...
import costService from "@/services/costCenter";
...
export type costCenterActionCallbacks = {
  ...
  return [
    {
      ...
      element: (item: { id: number; name: string }) => (
        <Link
          href={`/settings/cost-center/${item.id}/edit`}
    ...
    },
    {
      ...
      element: (
        ...
        { onDelete }: Pick<costCenterActionCallbacks, "onDelete">
      ) => (
        <ActionButton
          ...
          services={{
            deleteClient: async (id: number) => {
              try {
                let response = await costService.deleteCostCenter(id);
                onDelete(id);
