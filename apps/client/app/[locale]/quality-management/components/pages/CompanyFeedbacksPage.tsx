...
const CompanyFeedbacksPage: React.FC = () => {
  ...
  const [filterState, setFilterState] = React.useState<{
    company: FilterOption | null;
    evt: FilterOption | null;
    project: FilterOption | null;
    companyFeedback: FilterOption | null;
  }>({
    company: null,
    evt: null,
    project: null,
    ...
    async (page: number = 1) => {
      ...
      try {
        if (shiftType === "regular") {
          ...
          const transformedData: CompanyFeedbackRow[] = shifts.map(
            (shift: any) => {
              ...
              const dateISO = shift.date
                ...
                : undefined;
              const companyRatingRaw = shift.company_rating;
              const evtName =
              ...
              return {
                ...
                hasCompanyFeedback: normalizeRating(companyRatingRaw) > 0,
              };
            }
          );
  ...
  const filteredRows = React.useMemo(() => {
    return rows.filter((row) => {
      const { company, evt, project, companyFeedback } = filterState;

      if (company?.value !== undefined && company?.value !== null) {
        if (typeof company.value === "number") {
          if (row.companyId !== company.value) {
            return false;
          }
        } else if (
          row.company.toLowerCase() !== String(company.value).toLowerCase()
        ) {
          return false;
        }
      }
      ...
      if (companyFeedback?.value) {
        if (
          companyFeedback.value === "has-feedback" &&
          !row.hasCompanyFeedback
        ) {
          return false;
        }
        if (
          companyFeedback.value === "missing-feedback" &&
          row.hasCompanyFeedback
        ) {
          return false;
        }
  ...
  const companyOptions = React.useMemo<FilterOption[]>(() => {
  ...
  const columns: TableColumn[] = [
    ...
    { header: t("company"), accessor: "company", className: "w-[160px]" },
        ...
        headerBottomContent={
          <div className="flex flex-wrap items-center gap-3 px-4 md:px-0 py-2">
            <FilterPill
              ...
              onChange={(option) => {
                setFilterState((prev) => ({ ...prev, company: option }));
                resetPagination();
              }}
            />
            <FilterPill
            ...
            <FilterPill
              ...
              value={filterState.companyFeedback}
              onChange={(option) => {
                setFilterState((prev) => ({
                  ...prev,
                  companyFeedback: option,
                }));
                resetPagination();
              }}
            />
...
export default CompanyFeedbacksPage;
